...
export class Method {
	...
	async init(options: Record<string, boolean>) {
		...
		const methodsResponse: MethodsResponse = await pb
			.collection('methods')
			.getOne(this.id, { expand: expandString });
		if (!methodsResponse) throw new Error('Method not found in database');
		this.setPropertiesFromMethodsResponse(methodsResponse);

		if (options.elements)
			this.populateElements(
				methodsResponse.expand?.elements as ElementsResponse[],
				methodsResponse.expand?.units as UnitsResponse[]
			);
		if (options.blanks) this.populateBlankMap(methodsResponse.expand?.blanks as BlanksResponse[]);
		if (options.checkStandards)
			this.populateCheckStandardsMap(
				methodsResponse.expand?.checkStandards as CheckStandardsResponse[]
			);
		if (options.referenceMaterials)
			this.populateReferenceMaterialMap(
				methodsResponse.expand?.referenceMaterials as ReferenceMaterialsResponse[]
			);
	}
	...
	async addElement(elementId: string) {
		...
		this.elements =
			this.elements && this.elements.length > 0 ? [...this.elements, addedAnalyte] : [addedAnalyte];
	}
	...
	async createNewBlank(name: string) {
		...
		for (const currentElement of this.elements ?? []) {
			...
			const newDetectionLimits: DetectionLimitsResponse = await pb
				.collection('detectionLimits')
				.create(dlData);
		...
		const updatedNewBlank: BlanksResponse = await pb
			.collection('blanks')
			.update(newBlank.id, { detectionLimits: detectionLimitsIds }, { expand: 'detectionLimits' });
	...
	async addDetectionLimitsToBlanks(elementId: string) {
		...
		for (const blank of blanks.values()) {
			...
			const dlIdList =
				blank.detectionLimits && blank.detectionLimits.length > 0
					? [...blank.detectionLimits, newDl.id]
					: [newDl.id];
			...
			const updatedBlank: BlanksResponse = await pb
				.collection('blanks')
				.update(blank.id, updatedDls, { expand: 'detectionLimits' });
	...
	async addRangestoReferenceMaterials(elementId: string) {
		...
		for (const referenceMaterial of referenceMaterials.values()) {
			...
			const newRange: ReferenceMaterialsRangesResponse = await pb
				.collection('referenceMaterialsRanges')
				.create(rangeData);
			if (!newRange) throw new Error('Error saving range');
			//    update list of detection limits
			const rangesIdList =
				referenceMaterial.ranges && referenceMaterial.ranges.length > 0
					? [...referenceMaterial.ranges, newRange.id]
					: [newRange.id];
			...
			const updatedReferenceMaterial: ReferenceMaterialsResponse = await pb
				...
				.update(referenceMaterial.id, updatedRanges, { expand: 'ranges' });
	...
	async addValuesToCheckStandards(elementId: string) {
		...
		for (const checkStandard of checkStandards.values()) {
			...
			//    update list of detection limits
			const valuesIdList =
				checkStandard.checkValues && checkStandard.checkValues.length > 0
					? [...checkStandard.checkValues, newValue.id]
					: [newValue.id];
			...
			const updatedCheckStandard: CheckStandardsResponse = await pb
				...
				.update(checkStandard.id, updateCheckValues, { expand: 'checkValues' });
	...
	async removeDetectionLimitsFromBlanks(elementId: string) {
		...
		// for each blank
		for (const blank of blanks.values()) {
			const detectionLimits: DetectionLimitsResponse[] =
				(blank.expand?.detectionLimits as DetectionLimitsResponse[]) ?? [];
			...
			const deletedDetectionLimit = await pb
				.collection('detectionLimits')
				.delete(detectionLimit.id);
	...
	async removeRangesFromReferenceMaterials(elementId: string) {
		...
		for (const referenceMaterial of referenceMaterials.values()) {
			const ranges: ReferenceMaterialsRangesResponse[] =
				(referenceMaterial.expand?.ranges as ReferenceMaterialsRangesResponse[]) ?? [];
	...
	async removeValuesFromCheckStandards(elementId: string) {
		...
		// for each check
		for (const checkStandard of checkStandards.values()) {
			const checkValues = (checkStandard.expand?.checkValues as CheckValuesResponse[]) ?? [];
			//    find the values of interest
	...
	async deleteReferenceMaterial(referenceMaterialName: string) {
		...
		if (!this.referenceMaterials?.has(referenceMaterialName))
			throw new Error('Could not find reference material');
		...
		const deletedReferenceMaterial = await pb
			.collection('referenceMaterials')
			.delete(referenceMaterial.id);
	...
	async deleteCheckStandard(checkStandardName: string) {
		...
		if (!this.checkStandards?.has(checkStandardName))
			throw new Error('Error: could not find blank');
	...
	async updateReferenceRanges(rangeId: string, toUpdate: 'lower' | 'upper', value: number) {
		...
		const updatedRange: ReferenceMaterialsRangesResponse = await pb
			.collection('referenceMaterialsRanges')
			.update(rangeId, JSON.stringify(dataObject));
	...
	async updateDetectionLimits(detectionLimitsId: string, toUpdate: 'mdl' | 'loq', value: number) {
		...
		const dataObject: Record<string, number> = {};
		dataObject[toUpdate] = value;
		const updatedDetectionLimit: DetectionLimitsResponse = await pb
			.collection('detectionLimits')
			.update(detectionLimitsId, JSON.stringify(dataObject));
	...
	async updateCheckStandardName(checkStandardId: string, newName: string) {
		if (!this.checkStandards || this.checkStandards.size === 0)
			throw new Error('No check standard in method object');
	...
	async updateReferenceMaterialName(referenceMaterialId: string, newName: string) {
		if (!this.referenceMaterials || this.referenceMaterials.size === 0)
			throw new Error('No reference material in method object');
		const data = JSON.stringify({ name: newName });
	...
	async updateBlankName(blankId: string, newName: string) {
		...
		const updatedBlank: BlanksResponse = await pb.collection('blanks').update(blankId, data);
	...
	async createNewCheckStandard(name: string) {
		...
		const newCheckStandard: CheckStandardsResponse = await pb
			.collection('checkStandards')
			.create(data);
		...
		for (const currentElement of this.elements ?? []) {
			...
			const newValue: CheckValuesResponse = await pb
				.collection('checkValues')
				.create(checkValueData);
			if (!newValue) throw new Error('Error adding detection limits');
			checkValuesIds = [...checkValuesIds, newValue.id];
		}
		try {
			const updatedNewCheckStandard: CheckStandardsResponse = await pb
				.collection('checkStandards')
				.update(newCheckStandard.id, { checkValues: checkValuesIds }, { expand: 'checkValues' });
	...
	async createNewReferenceMaterial(name: string) {
		...
		for (const currentElement of this.elements ?? []) {
			...
			const range: DetectionLimitsResponse = await pb
				.collection('referenceMaterialsRanges')
				.create(rangeData);
		...
		const updatedNewReferenceMaterial: ReferenceMaterialsResponse = await pb
			.collection('referenceMaterials')
			.update(newReferenceMaterial.id, { ranges: rangeIds }, { expand: 'ranges' });
	...
	getValue(sampleType: SampleType, name: string, expandType: ExpandType, element: string) {
		...
		const targetElements = (targetSample.expand[expandType] as CheckValuesResponse[]).find(
			(x) => x.element === element
		);
	...
	getLowestLoqs() {
		...
		for (const blank of blanks.values()) {
			const detectionLimits = blank.expand?.detectionLimits as DetectionLimitsResponse[];
			for (const dl of detectionLimits) {
				...
				else if (lowestLoqs.get(mass).loq > (dl.loq ?? 0)) lowestLoqs.set(mass, dl.loq);
			}
		}
		return lowestLoqs;
	}
